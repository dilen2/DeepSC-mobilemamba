...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    scan_t snr = {params.snr, params.snr};
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                if (threadIdx.x == 0) {
                    if(state_idx == 0 ) {
                        ...
                        // smem_running_prefix[state_idx + r * Ktraits::MaxDState].x = params.snr;
                        // x[(r * params.n_chunks + chunk) * params.dstate + state_idx].x = params.snr;
                        smem_running_prefix[state_idx + r * Ktraits::MaxDState] = snr;
                        x[(r * params.n_chunks + chunk) * params.dstate + state_idx] = snr;
                    }
                    else{
                        smem_running_prefix[state_idx + r * Ktraits::MaxDState] = prefix_op.running_prefix;
                        x[(r * params.n_chunks + chunk) * params.dstate + state_idx] = prefix_op.running_prefix;
